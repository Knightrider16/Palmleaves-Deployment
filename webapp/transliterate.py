...
def run(
...
) -> tuple[list[list[str]], dict[str, np.ndarray], dict[str, str]]:
    """
    ...
    Args:
        ...
        use_realesrgan: If True, attempt Real-ESRGAN upscaling (may fall back to
                        Bicubic if unavailable). If False, use Bicubic directly.

    Returns:
        Tuple of (lines, stages, metadata) where:
        - lines: List of per-line token lists
        - stages: Dictionary of preprocessing stage images
        - metadata: Dict with 'upscale_method' and 'upscale_warning' keys
    """
    ...
    stages, metadata = preprocess.full_pipeline_with_stages(
        raw, use_realesrgan=use_realesrgan
    )
    mask = stages['cleaned']
