...
try:
    ...
    print(f"Warning: Real-ESRGAN not available. Import error: {e}")
except Exception as e:
    REALESRGAN_AVAILABLE = False
    print(f"Warning: Real-ESRGAN not available. Unexpected error: {e}")
...
def full_pipeline_with_stages(img: np.ndarray, use_realesrgan: bool = True) -> tuple[dict[str, np.ndarray], dict[str, str]]:
    ...
    3. Upscaled (2x Real-ESRGAN super-resolution, or bicubic fallback)
    ...
    Args:
        img: Input image (BGR, grayscale, or binary)
        use_realesrgan: If True, attempt Real-ESRGAN; falls back to Bicubic if
                        unavailable. If False, use Bicubic directly.

    Returns:
        Tuple of (stages_dict, metadata_dict) where:
        - stages_dict has keys: 'original', 'clahe', 'upscaled', 'sharpened', 'binary', 'cleaned'
        - metadata_dict has:
            'upscale_method'  : human-readable method name
            'upscale_warning' : non-empty string if a fallback occurred, else None
    """
    stages = {}
    ...
    if use_realesrgan:
        if upsampler is not None:
            ...
            metadata['upscale_warning'] = None
        else:
            # Real-ESRGAN was requested but is not available — fall back silently
            upscaled = lightweight_upscale(clahe, scale=2)
            metadata['upscale_method'] = 'Bicubic 2x'
            metadata['upscale_warning'] = (
                'Real-ESRGAN is not available in this environment (missing weights or '
                'package). Bicubic upscaling was used instead. Results may differ '
                'slightly from the full offline pipeline.'
            )
    else:
        upscaled = lightweight_upscale(clahe, scale=2)
        metadata['upscale_method'] = 'Bicubic 2x'
